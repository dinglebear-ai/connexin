...
async function fixture(overrides: Partial<RuntimeConfig> = {}) {
  const ptys: FakePty[] = [];
  const runtimeConfig = testRuntimeConfig({ maxWsPayloadBytes: 32, ...overrides });
...
function wsUrl(baseUrl: string, sessionId: string, token: string): string {
  return `${baseUrl.replace("http:", "ws:")}/terminal?session=${sessionId}&token=${token}`;
}

async function connectAndReadReady(url: string): Promise<WebSocket> {
...
describe("startBridgeServer", () => {
  ...
  it("can advertise a public bridge URL while listening locally", async () => {
    const { bridge, manager } = await fixture({ bridgePublicUrl: "https://quick-shell.example" });
    try {
  ...
  it("rejects missing or bad session tokens with policy violation", async () => {
    const { bridge, manager, session } = await fixture();
    const ws = new WebSocket(wsUrl(bridge.baseUrl, session.id, "bad"));
  ...
  it("rejects configured disallowed origins", async () => {
    const { bridge, manager, session } = await fixture({ allowedOrigins: ["https://allowed.example"] });
    const ws = new WebSocket(wsUrl(bridge.baseUrl, session.id, session.wsToken), {
  ...
  it("starts the SSH PTY lazily when the bridge socket connects", async () => {
    ...
    const ws = await connectAndReadReady(wsUrl(bridge.baseUrl, session.id, session.wsToken));
  ...
  it("rejects invalid JSON messages with an error response", async () => {
    ...
    await expect(nextMessage(ws)).resolves.toEqual({ type: "error", message: "invalid terminal message" });
    ws.close();
    manager.closeAll();
    await bridge.close();
  });
  ...
  it("enforces input length and resize bounds", async () => {
    const { bridge, manager, session } = await fixture({ maxWsPayloadBytes: 100 });
    const ws = await connectAndReadReady(wsUrl(bridge.baseUrl, session.id, session.wsToken));
    ...
    const ws2 = await connectAndReadReady(wsUrl(bridge.baseUrl, session.id, session.wsToken));
    ws2.send(JSON.stringify({ type: "resize", cols: 1, rows: 1 }));
    await expect(nextMessage(ws2)).resolves.toEqual({ type: "error", message: "invalid terminal message" });
    ws2.close();
    manager.closeAll();
    await bridge.close();
  });
  ...
  it("allows only one active socket per session by closing the old socket", async () => {
    const { bridge, manager, session } = await fixture();
    const url = wsUrl(bridge.baseUrl, session.id, session.wsToken);
    const first = await connectAndReadReady(url);

    const firstClosed = waitForClose(first);
    const second = await connectAndReadReady(url);
  ...
  it("ignores replaced socket input and disposes its PTY forwarding immediately", async () => {
    ...
    first.emit("message", Buffer.from(JSON.stringify({ type: "input", data: "stale" })));
    expect(ptys[0]?.writes).toEqual([]);

    ptys[0]?.emitData("fresh");
    await expect(nextMessage(second)).resolves.toEqual({ type: "output", data: "fresh" });
  ...
  it("forwards PTY output to the socket", async () => {
    ...
    await expect(nextMessage(ws)).resolves.toEqual({ type: "output", data: "hello" });
    ws.close();
    manager.closeAll();
    await bridge.close();
  });
  ...
  it("closes slow sockets and the session before a single output payload exceeds the limit", async () => {
    const { bridge, manager, ptys, session } = await fixture({ wsBufferedAmountLimitBytes: 32 });
    const ws = await connectAndReadReady(wsUrl(bridge.baseUrl, session.id, session.wsToken));
    const closed = waitForClose(ws);
  ...
  it("closes slow sockets and the session when queued output plus a new chunk exceeds the limit", async () => {
    const { bridge, manager, ptys, session } = await fixture({ wsBufferedAmountLimitBytes: 64 });
    const ws = await connectAndReadReady(wsUrl(bridge.baseUrl, session.id, session.wsToken));
    const originalBufferedAmount = Object.getOwnPropertyDescriptor(WebSocket.prototype, "bufferedAmount");
    Object.defineProperty(WebSocket.prototype, "bufferedAmount", { configurable: true, get: () => 50 });
    const closed = waitForClose(ws);
  ...
  it("closes active sockets when session cleanup closes the session", async () => {
    const { bridge, manager, ptys, session } = await fixture({ maxSessionAgeMs: 1 });
    const ws = await connectAndReadReady(wsUrl(bridge.baseUrl, session.id, session.wsToken));
    const closed = waitForClose(ws);
    session.createdAt = 0;
  ...
  it("ignores input and resize after PTY exit", async () => {
    ...
    const ws = await connectAndReadReady(wsUrl(bridge.baseUrl, session.id, session.wsToken));
    ptys[0]?.emitExit(0);
    await expect(nextMessage(ws)).resolves.toEqual({ type: "exit", exitCode: 0 });
